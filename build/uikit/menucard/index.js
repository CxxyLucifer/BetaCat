...
export default class Index extends Component {
    ...
    render() {
        ...
        <View style={styles.nameView}>
          ...
          {!isEmpty(data.childs) && data.childs.length > 4 ? (<TouchableOpacity onPress={this._showMore} style={[styles.flex1, styles.more]}>
              <Text style={{ color: '#9b9b9b', fontSize: 12 }}>{showMore ? '收起更多' : '展开更多'}</Text>
            </TouchableOpacity>) : (<View style={styles.flex1}/>)}
        </View>
        <View style={styles.menuView}>{this._renderMenu(data)}</View>
      </View>);
    }
}
